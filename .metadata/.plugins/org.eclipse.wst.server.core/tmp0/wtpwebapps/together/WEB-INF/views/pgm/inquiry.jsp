...
<script type="text/javascript">
   //QnA 댓글 입력
   $(function() {
	  $('#QnA').load('QnAList.do?pgm_num=${pgmInfo.num}');
  	  $('#qInsert').click(function() {
     	 if (!frm1.replytext.value) {
        	 alert("댓글 입력 후에 확인을 누르세요");
        	 frm1.replytext.focus(); return false;
     	 }
      	 var frmData = $('#frm1').serialize();
      	 $.post('qInsert.do',frmData, function(data) {
         	$('#QnA').html(data);
...
<!-- ==========================================Q&A=============================================== -->
				...
				<div class="f3_list f3_2_hid">
					<%-- <c:if test=""><!-- 사용자인경우 --> --%>
					<div class="qna_dix" align="left">
						<form name="frm1" id="frm1">
							<input type="hidden" name="pgm_num" value="${pgmInfo.num}">
							<input type="hidden" name="replyer" value="${member.nick_name}">
								<table>
								<c:if test="${result==0}">
									<tr>
										<th><h2>로그인 후에 사용해주세요</h2></th>
									</tr>
								</c:if>
								<c:if test="${result>0}">
									<tr>
										<td>질문글: <textarea rows="5" cols="90" name="replytext"
												required="required"></textarea></td>
									</tr>
									<tr>
										<td colspan="2" align="center"><input type="button"
											value="등록" id="qInsert"></td>
									</tr>
								</c:if>
							</table>
						</form>
