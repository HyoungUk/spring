			...
			<c:if test="${pp.total > 0 }">
				<c:forEach var="tboard" items="${list}">
					<tr>
						...
						<c:if test="${tboard.del!='y'}">
							<td title="${tboard.content}">
								<c:if test="${tboard.re_level > 0 }">
									<img alt="" src="images/level.gif" height="2" width="${tboard.re_level*15}">
									<img alt="" src="images/re.gif">
								</c:if> 
									<a class="btn btn-success btn-sm" href="tboardView.do?t_num=${tboard.t_num}&pageNum=${pp.currentPage}"> ${tboard.subject}</a> 
								<c:if test="${tboard.readcount > 30 }">
